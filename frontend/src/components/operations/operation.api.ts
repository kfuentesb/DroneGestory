...
export async function fetchNextOperationName() {
  const response = await apiFetch(`${API_BASE_URL}/api/operations/next-name`, {
    headers: { "Content-Type": "application/json" },
  });
  ...
  return (await response.json()) as { nombreAsignado: string };
}
