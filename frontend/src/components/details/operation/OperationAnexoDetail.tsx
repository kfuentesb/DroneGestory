...
export default function OperationAnexoDetail({ tipoAnexo }: OperationAnexoDetailProps) {
  ...
  const handleExternalPersonnelChange = (nextPersonnel: ExternalPersonnelSignature[]) => {
    ...
    setAnexoData((prev) => {
      ...
      return {
        ...(prev as Anexo5Data),
        externalPersonnel: nextPersonnel,
      } as AnexoData;
    });
  };
  ...
  const buildAnexo5Data = (
    ...
    if (!anexo5 && !anexo4) {
      ...
      return mergedExternal.length ? ({ externalPersonnel: mergedExternal } as Anexo5Data) : null;
    }

    if (!anexo5 && anexo4) {
      const created: Anexo5Data = {
  ...
  const hasAircraftSelectionForAnexo = !requiresAircraftSelection || anexoAircraftOptions.length > 0;
        ...
        if (selectedVersionId) {
          switch (tipoAnexo) {
            case 4:
              ...
              break;
            case 5: {
              const [anexo5Version, anexo4] = await Promise.all([
          ...
          switch (tipoAnexo) {
            case 4:
              ...
              break;
            case 5: {
              const [anexo5, anexo4] = await Promise.all([
        ...
        <div className="d-flex justify-content-between align-items-center">
          ...
          <div className="d-flex gap-2">
            ...
            {canEditDraft && !isViewingHistoricalVersion && (
              ...
              >
                <img src={saveIcon} alt="" aria-hidden="true" className="d-inline d-md-none" style={{ width: 16, height: 16 }} />
                <span className="d-none d-md-inline">{saving ? "Guardando..." : "Guardar Borrador"}</span>
              </ButtonProp>
            )}
          </div>
        </div>
          ...
          <div className="card-body">
            ...
            {anexoAircraftOptions.length === 0 ? (
              <div className="text-muted small mb-0">
                Selecciona al menos una aeronave en Anexo 4 para continuar.
              </div>
            ) : (
              <select
            ...
            <>
              ...
              {tipoAnexo === 5 && (
                <FormOperationAnexo5Detail
                  ...
                  disabled={isViewingHistoricalVersion || !canEditDraft || saving}
                  onExternalPersonnelChange={handleExternalPersonnelChange}
                  readOnlyMessage={
                    isViewingHistoricalVersion
                      ? "Estás consultando una versión histórica. Esta vista es solo lectura."
                      : undefined
